...
import playground.common.messaging.idempotency.publish
import playground.common.messaging.publisher.Topic
import playground.common.rest.idempotency.AutowireSetting
import playground.common.rest.idempotency.IdempotentHandler
...
class HtmlController(
    ...
    private val logger: Logger = LoggerFactory.getLogger(HtmlController::class.java)
    ...
    @PostMapping("/publish")
    @IdempotentHandler(
        processId = "HtmlController-publishCustomer",
        lockDurationMs = 1000
    )
    @AutowireSetting(PublishingInFinalPhase::class)
    fun publishCustomer(): String {
        with(IdempotencyContextHolder.get()) {
            ...
            this.publish(
                customerProfileSnapshotTopic,
                customerProfileSnapshot.customerId,
                customerProfileSnapshot,
                MessageMetadata(customerId = customerProfileSnapshot.customerId, idempotencyKey = idempotencyKey)
            )
//            eventPublisher.publishCustomerProfileSnapshot(customerProfileSnapshot)
        }
        // IdempotencyContextHolder.get().publish(someTopic, customerId, responseMessage, MessageMetadata(customerId = customerId, idempotencyKey = idempotencyKey))
